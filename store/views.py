...
def products_view(request):
    try:
        ...
        products = Product.objects.none() if hasattr(Product, 'objects') else []
        categories = []
        all_categories = []
        return render(request, 'store/products.html', {
            'products': [], 'categories': [], 'all_categories': [],
            'query': '', 'selected_category': '', 'price_min': '', 'price_max': '',
            'sort': '', 'tab': '', 'cart_item_count': 0,
        })
    ...
    if query:
        products = products.filter(Q(name__icontains=query) | Q(description__icontains=query) | Q(brand__icontains=query) | Q(category__name__icontains=query))
    if category_slug:
    ...
    cart = get_cart(request)
    return render(request, 'store/products.html', {
        'products': page_obj, 'categories': categories, 'all_categories': all_categories,
        'query': query, 'selected_category': category_slug, 'price_min': price_min,
        'price_max': price_max, 'sort': sort, 'tab': tab,
        'cart_item_count': cart.get_item_count(),
    })
...
def product_detail_view(request, slug):
    ...
    return render(request, 'store/product_detail.html', {
        ...
        'cart_form': CartAddForm(), 'cart_item_count': cart.get_item_count(),
    })
...
def categories_view(request):
    ...
    return render(request, 'store/categories.html', {
        'categories': categories, 'cart_item_count': cart.get_item_count(),
    })


def category_products_view(request, slug):
    category = get_object_or_404(Category, slug=slug)
    products = Product.objects.filter(Q(category=category) | Q(category__parent=category), is_active=True)
    sort = request.GET.get('sort', '')
    if sort == 'price_low':
        products = products.order_by('price')
    elif sort == 'price_high':
    ...
    return render(request, 'store/category_products.html', {
        'category': category, 'products': page_obj, 'sort': sort,
        'cart_item_count': cart.get_item_count(),
    })
...
def add_to_cart_view(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({'success': True, 'cart_count': cart.get_item_count(), 'message': f'{product.name} added to cart!'})
...
def cart_view(request):
    cart = get_cart(request)
    cart_items = cart.items.select_related('product').all() if not isinstance(cart, _EmptyCart) else []
    cart_total = cart.get_total()
    delivery_fee = 200
    return render(request, 'store/cart.html', {
        'cart': cart, 'cart_items': cart_items, 'cart_total': cart_total,
        'delivery_fee': delivery_fee, 'grand_total': cart_total + delivery_fee,
        'cart_item_count': cart.get_item_count(),
    })
...
def checkout_view(request):
    ...
    if request.method == 'POST':
        ...
        form = CheckoutForm(initial={
            ...
            'email': request.user.email, 'city': 'Lahore',
        })
...
def order_confirmation_view(request, order_id):
    ...
    return render(request, 'store/order_confirmation.html', {
        'order': order, 'cart_item_count': get_cart(request).get_item_count(),
    })
...
def order_history_view(request):
    ...
    return render(request, 'store/order_history.html', {
        'orders': orders, 'cart_item_count': get_cart(request).get_item_count(),
    })
